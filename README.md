...
+ 2주차: 계획대로 스테이지 객체를 만들고 10개의 스테이지를 생성해서 플레이어와 몬스터의 위치를 정함 - 100% 완료  
	...
	+ Scene 상태 다이어그램도 그렸지만, 수정이 필요함.
	+ Stage를 Scene으로 관리하지 않음.
	+ 10개의 스테이지 구성 후 스테이지 생성 완료

+ 3주차: 일시정지 메뉴 구현 x, 아이템 구현 x. 스킬 구현도 부족하다. - 70% 완료
	+ 배경 Scroll 가능(패럴랙스 스크롤링)
	+ Title Scene 구현
	+ 스테이지 구성은 전부 완료, 아이템만 추가하면 됨.

+ 4주차: 보스 구현 x, save/load x, 이미지/사운드 x - 30% 완료
	+ 몬스터 인공지능 구현 완료
...
+ Stage
	...
	+ tmx 파일에 포함되어 있는 오브젝트들은 floor 클래스와 monster 클래스다.
	+ 각 스테이지에 스테이지 전환 트리거를 넣어 플레이어가 트리거의 위치에 가게 되면 스테이지가 전환된다.

+ Player
	+ Player 클래스는 a와 d키로 기본적인 움직임, space로 jump가 가능하다. 현재는 더블 점프까지 할 수 있다.
	+ Attack이란 클래스로 몬스터를 공격할 수 있다. 공격은 좌클릭으로 좌클릭을 할 때마다 PlayerAttack 클래스를 생성.
	+ PlayerAttack에는 일반공격과 스킬이 모두 포함되어 있다.
	+ 몬스터를 잡게 되면 몬스터로부터 재화를 받을 수 있다.
	+ 일정 경험치를 얻게 되면 레벨 업을 하고 SP를 얻는다. SP는 능력치를 올릴 수 있는데 사용한다.(아직 SP 관련 시스템 구현 X)
